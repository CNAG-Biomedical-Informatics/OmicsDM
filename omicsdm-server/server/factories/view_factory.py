...
class ViewSpecificFields:
    ...
    def get_fields(self, table_name):
        ...
        if table_name not in ["PROJECTS", "ANALYSIS"]:
    ...
    def table_FILES(self):
        ...
        # modify the returned json to tell
        # the frontend to show the cellxgene button
        if self.query_result_mapped["name"].endswith(".h5ad"):
            view_specific_fields["visualizer"] = "cellxgene"
    ...
    def table_ANALYSIS(self):
        # TODO needs to be covered by a test

        dataset_ids = [
            Dataset.find_by_id(ds_id).dataset_id
            for ds_id in self.query_result.dataset_ids
        ]

        group_query = (
            self.group_query.join(
                Analysisgroups, Group.id == Analysisgroups.group_id
            )
            .join(Analyses, Analyses.id == Analysisgroups.analysis_id)
            .filter(self.table.analysis_id == self.query_result.analysis_id)
        )

        return (
            {
                "id": self.query_result_mapped["analysis_id"],
                "analysis_name": self.query_result_mapped["name"],
                "dataset_ids": ",".join(dataset_ids),
                "file_ids": ",".join(
                    map(str, self.query_result_mapped["file_ids"])
                ),
                # "analysis_settings":
                # json.dumps(self.query_result_mapped["analysis_settings"]),
            },
            group_query,
            self.ds_query_result,
            self.query_result_mapped,
...
def data_view_builder(users_group_name, table, table_name, request, ns):
    ...
    # additional meta data needed for the following react library:
    # https://www.material-react-table.com/
    res["meta"] = {}
    res["meta"]["totalRowCount"] = 0
    ...
    if not admin:
        ...
        conditions = [Groups.group_id == users_group_id]
        # this should work for the ANALYSIS table as well
        if table_name != "ANALYSIS":
            conditions.extend(
                [
    ...
    if table_name in ["DATASETS", "FILES"] and not admin:
