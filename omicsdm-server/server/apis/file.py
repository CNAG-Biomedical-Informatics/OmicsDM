...
@api.expect(parser)
@ns.route("/adminviewcols", methods=(["GET"]))
...
class RequestSigner(Resource):
    ...
    def get(self, userid, groups):
        ...
        if ceph_obj_key[3] not in ["dataPolicy", "clinical"]:
            ...
            file_name, file_version = ceph_obj_key[3].split("_uploadedVersion_")
        ...
        signature = base64.b64encode(
            hmac.new(secret, to_sign_str.encode("utf-8"), hashlib.sha1).digest()
        )
...
class FileFinishUpload(Resource):
    ...
    def post(self, userid, groups):
        ...
        if group is None:
            ...
            return make_response(jsonify({"message": "user not authorized"}), 405)
        ...
        if dataset is None:
            return make_response(jsonify({"message": "dataset does not exist"}), 404)
        ...
        if file_query is None:
            return make_response(jsonify({"message": "file does not exist"}), 404)
...
class FileDownload(Resource):
    ...
    def post(self, userid, groups):
        ...
        schema = get_obj_schema({"file_ids": {"type": "array"}}, required=["file_ids"])
        ...
        for file_id in data["file_ids"]:
            ...
            if file_dl_allowed or file_owner_group == group_name:
                url = create_presigned_url(app.config, "get_object", *file_query)
...
class FileDisable(Resource):
    ...
    def post(self, userid, groups):
        ...
        schema = get_obj_schema({"fileIds": {"type": "array"}}, required=["fileIds"])
        ...
        if len(files) != len(file_ids):
            raise EmptyQuery(query, "at least one of the requested files was not found")
...
class FileDataStartUpload(Resource):
    ...
    def post(self, userid, groups):
        ...
        if group is None:
            ...
            # has never interacted with the server before
            # => blocks possible malicious actions
            return make_response(jsonify({"message": "user not authorized"}), 404)

        data = request.get_json()

        props = {
            k: {"type": "string", "minLength": 1} for k in app.config["FILE_FIELDS"]
        }
        ...
        if file_extension not in app.config["ALLOWED_FILE_EXTENSIONS"]:
            raise BadFile(data, f"file {file_name} contains forbidden file extension")
...
class FileCellxgene(Resource):
    ...
    def post(self, userid, groups, token):
        ...
        # make sure that file name has the correct file ending
        if data["file_name"].endswith(".h5ad") is False:
            return make_response(
        ...
        # TODO
        # add Tests
        if file_exists(data, group):

            # start a docker container on shinyproxy
            # return the url to the user

            # create a launch script and write it into cxg_mountpoint

            owner = data["dataset_owner"]  # keycloak group name
            ds = data["dataset_id"]
            fn = data["file_name"]
...
ns.add_resource(FilesAdminViewCols, "/viewcols", endpoint="files_vadmin_view_cols")

ns.add_resource(FileFinishUpload, "/finishupload", endpoint="file_finish_upload")
...
ns.add_resource(FileDataStartUpload, "/startupload", endpoint="file_data_start_upload")
ns.add_resource(FileSubmissionCols, "/submissioncols", endpoint="file_submission_cols")
ns.add_resource(FileCellxgene, "/cellxgene", endpoint="file_cellxgene")
