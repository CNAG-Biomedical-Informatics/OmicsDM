...
@api.expect(parser)
@ns.route("/visualize", methods=(["POST"]))
class DatasetsVisualization(Resource):
    """
    ...
    @api.doc(
        description="""
            Launch a cellxgene instance to visualize the dataset.
        """
    )
    @api.response(500, "Server error")
    @api.response(200, "Success")
    def post(self, userid, groups):
        ...
        try:
            url = deploy_cellxgene(
                userid, dataset_owner, dataset_id, file_name, file_version
            )
            return make_response(
                jsonify({"message": "New cellxgene instance up", "data": url}),
                200,
            )
        except MaxInstanceException:
            print("MaxInstanceException")
            return make_response(
                jsonify({"message": "Too many instances for this user."}), 500
            )
        except Exception:
...
@api.expect(parser)
@ns.route("/visualize/stop", methods=(["POST"]))
class DatasetsStopVisualizationInstance(Resource):
    """
    Dataset visualization
    """
    ...
    def post(self, userid, groups):
        ...
        try:
            # TODO
            # suis is expecting a manifest
            # but I don't know which one it is expecting?
            suis = KAH.list_custom_object()
            sui_instance_name = suis[0]["items"][0]["metadata"]["name"]
            KAH.delete_custom_object(name=sui_instance_name)

            # TODO
            # This is how I think it should be done
            # call KAH.list_custom_object()
            # to get what SUI the user has started, right?
            # loop over the suis to get the one which matches
            # my dataset_owner/id/file_name/version
            # get the instance name
            # suis[0]['items'][0]['metadata']['name']
            # And Finally I call
            # KAH.delete_custom_object(name=sui_instance_name)
            ...
            return make_response(
                jsonify(
                    {"message": "cellxgene instance stopped", "data": url}
                ),
                200,
            )
        except Exception:
...
ns.add_resource(
    DatasetsVisualization,
    "/visualize",
    endpoint="dataset_visualize_cxg",
)
