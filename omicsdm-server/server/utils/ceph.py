...
from server.utils.error_handler import ServiceUnavailable, NoSuchKeyOnS3
...
def create_presigned_url_for_analysis(
    config,
    method,
    key,
    expiration=3600,
...
def save_json(config, key, json_data):
    """
    save json file to ceph
    """

    print("ceph.save_json")
    client, bucket_name = get_client(config)
...
def get_file(config, key, previous_analysis_id):
    """
    get file from ceph
    """
    print("ceph.get_file")
    ...
    try:
        ...
        err_msg = "html not found for the previous analysis"
        if previous_analysis_id:
            raise NoSuchKeyOnS3(
                key,
                err,
                f"{err_msg} {previous_analysis_id}",
            )
        return None
