...
class WrongSchema(ApiException):
    ...
    def __init__(self, message, code=405):
        ...
        field_empty = "At least one required field is empty/unselected"

        error_mapping = {
            ".pdf$": "The data usage policy file must be in .pdf format",
            ".(csv|json)$": "The clinical file must be .csv or .json",
            "'^[^\\\\s]+$'": "The file name must not contain spaces",
            "d+$": "Samples or Features Count must be numeric",
            "'' is too short": field_empty,
            "is a required property": field_empty,
            "is not valid under any of the given schemas": field_empty,
        }

        self.message = message
        self.status_code = code
        self.message = error_mapping.get(message, message)

        for substring in error_mapping.keys():
            if substring in message:
                self.message = error_mapping.get(substring)
...
class BadArgument(ApiException):
    ...
    def __init__(self, arg):
        ...
        self.message = (
            f"{arg} not a valid argument please look into the documentation"
        )
        self.status_code = 405
...
class InvalidGroup(ApiException):
    ...
    def __init__(self, group):
        ...
        self.message = f"{group} is not a valid group"
        self.status_code = 405
...
class NoSuchKeyOnS3(ApiException):
    """
    Attributes:
    ...
    returns Error message when a file is not found on S3
    """

    def __init__(self, key, error, message, status_code=500, payload=None):
        self.key = (key,)
        self.error = (error,)
        self.message = message
        self.status_code = status_code
