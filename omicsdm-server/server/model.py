...
class Project(db.Model):
    """
    Contains the projects e.g. 3TR, PRECISES<ADS
...
class Analysisgroups(db.Model):
    """
    Link between tables analysis and table group.
    Provides permissions to analysis based on keycloak groups
    """
...
# Tables below are not used right now but
# need to be included in model.py otherwise Jenkins fails
class Analyses(db.Model):
    """
    Schema and functions for the table projects
    - id = autoincremented integer
    - analysis_id = Opal project id
    - name = Opal project name human readable
    - tags = Opal project tags
    - dataset_ids = list of used datasets
    - file_ids = list of used files
    - visibility = private to this project or visible to all
    - shared_with = kcloak groups which are able to see that analysis (0 = ALL)
    - submitter_name = keycloak id of the one who created the analyis
    - groups = keycloak groups of the 'submitter_name'
    - status = status of the analysis (starting/running/failed/finished)
    - jenkins_jobs = dictionary mapping jenkins job_ids to status
    """
    ...
    dataset_ids = Column(ARRAY(Integer), nullable=False)
    file_ids = Column(ARRAY(Integer), nullable=False)
    private = Column(Boolean, default=True)
    submitter_name = Column(String(120), nullable=False)
    submission_date = Column(DateTime, nullable=False, default=datetime.utcnow)
    shared_with = Column(ARRAY(Integer), nullable=False)
    status = Column(String(120), nullable=False)
    analysis_settings = Column(JSONB)
    jenkins_jobs = Column(JSONB)
    extra_cols = Column(JSONB, nullable=False)

    groups = db.relationship("Analysisgroups", cascade="all,delete")
...
class AnalysisTemplates(db.Model):
    # Only the admin should be able to add modify or delete templates

    """
    Schema and functions for the table analysis_template
    - id = autoincremented integer
    - name = name of the template
    - description = description of the template
    - analysis_type = RNA seq vs Single cell
    - based_on = list of analysis template ids
    - template = JSON containing the template
    """
    ...
    )  # RNA seq vs Single cell

    # ! when trying to add a analysis template which used the based on keyword
    # make sure that the template on which is based on is already in the db
    based_on = Column(
        ARRAY(Integer), nullable=False
    )  # list of analysis template ids
