          ...
          <Grid item>
            <Card
              sx={{
                ...
                border: showBorder(cardType, selectedCard, index) ? 'solid 1px blue' : 'white'
              }}
            >
              <CardActionArea
...
export default function ConfigureAnalysis(props) {
  ...
  const getFollowingCards = async (selectedCard, cardType, assignedFiles, previousAnalysisId) => {
  ...
  const handleClick2 = async (id, cardType, selectedCard, onChange, cardInfo) => {
  ...
  const handleClose = async (analysisId) => {
    ...
    const template = await getFollowingCards(selectedCard, selectedAnalysisLevel, assignedFiles, analysisId)
    console.log("template", template)
    console.log("update template", analysisJson)
  ...
  const handleUsePreviousClick = async (cardType, selectedCard) => {
    ...
    const fileNames = Object.keys(filesSelected).map(file => file.split("?").pop());
  ...
  const handleFileSelection = (event, fileType) => {
    ...
    const updatedAssignedFiles = { ...assignedFiles, [fileType]: event.target.value };
    console.log("updatedAssignedFiles", updatedAssignedFiles)
    setAssignedFiles(updatedAssignedFiles)
  }

  const renderFileSelection = (analysisObj, analysis, filesSelected, selectedCard) => {
    ...
    const renderReactomeMappingFileSelector = () => {
      ...
      return (
        <FormControl sx={{ m: 1, minWidth: 120 }}>
          <InputLabel id="select-1-label"> Reactome identifier mapping file </InputLabel>
          <Select
            onChange={(event) => handleFileSelection(event, "use_reactome_identifier_mapping_file")}
  ...
  const renderFileSelectionDialog = (analysisJson, selectedCard, filesSelected) => {
    console.log("filesSelected", filesSelected)
    ...
    const renderExplanation = (selectedCardOptions) => {
      if (Object.keys(selectedCardOptions.options).includes("use_reactome_identifier_mapping_file")) {
        return (
          "You can either select an uploaded gmt file or select a reactome identifier mapping file"
        )
      }
    }
    ...
    const disableConfirmButton = () => {
      ...
      const allKeysArePresent = requiredKeys.every(key => Object.keys(assignedFiles).includes(key))
    ...
    return (
      ...
      >
        ...
        <DialogContent>
          ...
          {renderFileSelection(analysisJson, selectedCard, filesSelected, selectedCard)}
        </DialogContent>
  ...
  return (
    <Grid container>
      <Grid item xs={6}>
        <br />
        <Typography variant={"h6"}>
          As a starting point please use the prefilled templates or take a finished analysis
        </Typography>
        <Tabs
          value={selectedTab}
        ...
        {open2 ? (
          ...
          >
            ...
            <DialogContent>
              ...
              <DialogContentText>
                If you want to use the output of the previous run,
                please select the corresponding analysis in the list of finished analyses.
              </DialogContentText>
              {renderAnalysesSelection(selectedAnalysisId)}
            </DialogContent>
            <DialogActions>
              <Button onClick={() => handleClose2(false, selectedAnalysisId)} autoFocus>
                use the output of the selected previous run
              </Button>
              <Button onClick={() => handleClose2(true)}>
                run it again
              </Button>
            </DialogActions>
          </Dialog>
